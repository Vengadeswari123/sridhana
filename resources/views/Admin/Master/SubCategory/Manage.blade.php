            ...
            <form method="post" enctype="multipart/form-data" class="p-2"> @csrf
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 mt-2">
                        <vue-inputvselect label="Category" name="catid" :options="{{ $options['category'] }}" :value="{{ old('category')??$data->data['catid']??1 }}" placeholder="Category"></vue-inputvselect>
                    </div>
                {{-- <div class="col-12">
                    <vue-admin-master-subcategory :options="{{ json_encode($options) }}" :catid="{{ json_encode(old('cat_id')??$data->data['cat_id']??'') }}" :subcatid="{{ json_encode(old('subcat_id')??$data->data['subcat_id']??'') }}"></vue-admin-master-subcategory>
                </div> --}}
