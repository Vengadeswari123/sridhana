...
class BeneficaryListGender implements FromCollection,WithHeadings,WithMapping
{
    ...
    public function collection()
    {
        ...
        $name=$this->name;
        if($did){
            $b=$b->where('personal->district', $did);
        }
        if($tid){
            $b=$b->where('personal->taluk', $tid);
        }
        if($vid){
            $b=$b->where('personal->village', $vid);
        }
        if($name){
            $b=$b->with('familymembersreport', function ($q) use ($name) {
                return $q->where('personal->gender', $name);
            })->where('personal->gender', $name);
        }
        return $b->get();
    }
