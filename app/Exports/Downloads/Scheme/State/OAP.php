...
{
    public $villageper,$dataper,$villagesel,$periodfrom,$periodto,$batch,$month;

    function __construct($dataper,$villagesel,$villageper,$periodfrom,$periodto,$batch,$month) {
    ...
    {
        $datax=Scheme::getdata('oap')->whereNotNull('meta->village')->orderBy('meta->village','asc');
        ...
        if(strlen($this->month)>0){
            $datax=$datax->whereIn('details->month',$this->month);
        }
        if($this->periodfrom){
    ...
    public function map($data): array
    {
        ...
        if($this->dataper->income){
            array_push($x,($data->details['income']??''));
        }
    ...
    public function headings(): array
    {
        ...
        if($this->dataper->amount){
            array_push($x,'Sanctioned Amount');
        }
        if($this->dataper->income){
            array_push($x,'Annual Income');
        }
        if($this->dataper->bankdetails){
