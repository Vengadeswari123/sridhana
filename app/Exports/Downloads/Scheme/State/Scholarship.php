...
class Scholarship implements FromCollection,WithHeadings,WithMapping
{
    ...
    {
        $datax=Scheme::getdata('cashaward')->whereNotNull('meta->village')->orderBy('meta->village','asc');
    ...
    public function map($data): array
    {
        ...
        if($this->dataper->schoolname){
            array_push($x,($data->details['schoolname']??''));
        }
        if($this->dataper->marks){
            array_push($x,($data->details['marks']??''));
            array_push($x,($data->details['total']??''));
            array_push($x,($data->details['percentage']??''));
        }
        if($this->dataper->opttext){
            array_push($x,($data->details['opttext']??''));
        }
    ...
    public function headings(): array
    {
        ...
        if($this->dataper->schoolname){
            array_push($x,'School Name');
        }
        if($this->dataper->marks){
            array_push($x,'Marks','Total','Percentage');
        }
        if($this->dataper->opttext){
            array_push($x,'Applied for cash award');
        }
        if($this->dataper->bankdetails){
