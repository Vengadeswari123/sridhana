...
class LANS implements ToModel, WithHeadingRow, SkipsEmptyRows
{
    public function model(array $row)
    {
        ...
        $scheme = 'lans';
        $url = '/admin/scheme/state/lans';
        $title = 'Life and Nutritional Support';
        $data = new Scheme();
        ...
        if ($benef) {
            ...
            $details['occupation']=$row['Occupation']??null;
            $details['annualfamilyincome']=$row['Annual Income']??null;
            $details['govtemployee']=$row['Govt Employee']??null;
            $details['govtemployeedetail']=$row['Govt Employee Details']??null;
            $details['grants']=$row['Grant Availed']??0;
            $details['grantsdetails']=$row['Grant Availed Details']??null;
